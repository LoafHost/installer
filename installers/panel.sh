...
MYSQL_USER="${MYSQL_USER:-loafpanel}"
...
panel_dl() {
  output "Downloading Loaf Panel files .. "
  mkdir -p /var/www/loaf-panel
  cd /var/www/loaf-panel || exit
  ...
  success "Downloaded Loaf Panel files!"
}
...
configure() {
  ...
  php artisan lp:environment:setup \
    --author="$email" \
    --url="$app_url" \
    --timezone="$timezone" \
    --cache="redis" \
  ...
  php artisan lp:environment:database \
    --host="127.0.0.1" \
    --port="3306" \
    --database="$MYSQL_DB" \
    --username="$MYSQL_USER" \
...
insert_cronjob() {
  ...
  crontab -l | {
    cat
    output "* * * * php /var/www/loaf-panel/artisan schedule:run >> /dev/null 2>&1"
  } | crontab -
...
php_fpm_conf() {
  curl -o /etc/php-fpm.d/www-loaf-panel.conf "$GITHUB_URL"/configs/www-loaf-panel.conf
...
configure_nginx() {
  ...
  ubuntu | debian)
    ...
    ;;
  rocky | almalinux)
    PHP_SOCKET="/var/run/php-fpm/loaf-panel.sock"
  ...
  curl -o "$CONFIG_PATH_AVAIL"/loaf-panel.conf "$GITHUB_URL"/configs/$DL_FILE

  sed -i -e "s@<domain>@${FQDN}@g" "$CONFIG_PATH_AVAIL"/loaf-panel.conf

  sed -i -e "s@<php_socket>@${PHP_SOCKET}@g" "$CONFIG_PATH_AVAIL"/loaf-panel.conf

  case "$OS" in
  ubuntu | debian)
    ln -sf "$CONFIG_PATH_AVAIL"/loaf-panel.conf "$CONFIG_PATH_ENABL"/loaf-panel.conf
    ;;
  esac
