...
#                                                                                    #
# https://github.com/loaf-panel/installer/blob/master/LICENSE #
#                                                                                    #
# This script is not associated with the official Loaf Panel Project.               #
# https://github.com/loaf-panel/installer                     #
#                                                                                    #
######################################################################################
...
export MYSQL_DBHOST_USER="loafpaneluser"
...
main() {
  ...
  if [ -d "/etc/loaf-panel" ]; then
    warning "The script has detected that you already have Loaf-Wings on your system! You cannot run the script multiple times, it will fail!"
  ...
  echo "* "
  echo "* The installer will install Docker, required dependencies for Loaf-Wings"
  echo "* as well as Loaf-Wings itself. But it's still required to create the node"
  ...
  echo "* official documentation: $(hyperlink 'https://docs.loaf-panel.com/wings/1.0/installing.html#configure')"
  echo "* "
  echo -e "* ${COLOR_RED}Note${COLOR_NC}: this script will not start Loaf-Wings automatically (will install systemd service, not start it)."
  ...
  if [ "$CONFIGURE_DBHOST" == true ]; then
    ...
    while [[ "$MYSQL_DBHOST_USER" == *"-"* ]]; do
      required_input MYSQL_DBHOST_USER "Database host username (loafpaneluser): " "" "loafpaneluser"
...
function goodbye {
  echo ""
  print_brake 70
  echo "* Loaf-Wings installation completed"
  echo "*"
  echo "* To continue, you need to configure Loaf-Wings to run with your panel"
  echo "* Please refer to the official guide, $(hyperlink 'https://docs.loaf-panel.com/wings/1.0/installing.html#configure')"
  echo "* "
  echo "* You can either copy the configuration file from the panel manually to /etc/loaf-panel/config.yml"
  echo "* or, you can use the \"auto deploy\" button from the panel and simply paste the command in this terminal"
  echo "* "
  echo "* You can then start Loaf-Wings manually to verify that it's working"
  echo "*"
  echo "* sudo wings"
  echo "*"
  echo "* Once you have verified that it is working, use CTRL+C and then start Loaf-Wings as a service (runs in the background)"
  echo "*"
