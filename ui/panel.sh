...
main() {
  ...
  if [ -d "/var/www/loaf-panel" ]; then
    warning "The script has detected that you already have Loaf Panel on your system! You cannot run the script multiple times, it will fail!"
  ...
  MYSQL_DB="-"
  while [[ "$MYSQL_DB" == *"-"* ]]; do
    required_input MYSQL_DB "Database name (loafpanel): " "" "loafpanel"
    [[ "$MYSQL_DB" == *"-"* ]] && error "Database name cannot contain hyphens"
  done

  MYSQL_USER="-"
  while [[ "$MYSQL_USER" == *"-"* ]]; do
    required_input MYSQL_USER "Database username (loafpaneluser): " "" "loafpaneluser"
  ...
  email_input email "Provide the email address that will be used to configure Let's Encrypt and Loaf Panel: " "Email cannot be empty or invalid"
  ...
  if [[ "$CONFIRM" =~ [Yy] ]]; then
    ...
    echo -e -n "\n* Would you like to install phpMyAdmin to manage the database? (y/N): "
    read -r CONFIRM_PMA
...
summary() {
  print_brake 62
  output "Loaf Panel $LOAF_PANEL_VERSION with nginx on $OS"
